...
//bool Findwindow::Set(int idx, WCHAR * findClass, WCHAR * findTitle,int findIdx)
//{
//	if (idx == -1) {
//		m_class.push_back(findClass);
//		m_title.push_back(findTitle);
...
//	}
//	else if (m_class.size() > idx) {
//		m_class[idx] =findClass;
//		m_title[idx] =findTitle;
//		m_idx[idx] = findIdx;
//	}else
//		return false;
//	return true;
...
void Findwindow::Set(int idx, int col, std::wstring buf)
{
	if (m_class.size() > idx) {
		...
		{
		case 0:
			m_class.push_back(buf);
			break;
		case 1:
			m_title.push_back(buf);
			break;
		case 2:
			m_idx.push_back(_wtoi(buf.c_str()));
			break;
		}
	}
}
