...
void Settings::Load()
{
	...
	GetPrivateProfileString(L"settings", L"DS4Serial", L"", m_serial, sizeof(m_serial)/ sizeof(m_serial[0]), m_file);
	...
	if (ret > 0 && ret - 2 != n) {
		...
		while (*key != 0) {
			...
			*value++ = 0;
			...
			if (mapdata[MapName::Mapping_DS4_1] && mapdata[MapName::Mapping_ButtonID]) {
				Mapping btn;
				...
				btn.DS4ID[0] = (DS4ButtonID) mapdata[MapName::Mapping_DS4_1];
				btn.DS4ID[1] = (DS4ButtonID) mapdata[MapName::Mapping_DS4_2];
				btn.DS4ID[2] = (DS4ButtonID) mapdata[MapName::Mapping_DS4_3];
				btn.DS4ID[3] = (DS4ButtonID) mapdata[MapName::Mapping_DS4_4];
				btn.Disbale[0] = mapdata[MapName::Mapping_DS4_1_Disable] == 1;
				btn.Disbale[1] = mapdata[MapName::Mapping_DS4_2_Disable] == 1;
				btn.Disbale[2] = mapdata[MapName::Mapping_DS4_3_Disable] == 1;
				btn.Disbale[3] = mapdata[MapName::Mapping_DS4_4_Disable] == 1;
...
void Settings::Save()
{
	...
	WritePrivateProfileString(TEXT("settings"), TEXT("DS4Serial"), m_serial, m_file);
	...
	for (int i = 0; i < length; i++) {
		...
		mapdata[MapName::Mapping_DS4_1] = btn->DS4ID[0];
		mapdata[MapName::Mapping_DS4_2] = btn->DS4ID[1];
		mapdata[MapName::Mapping_DS4_3] = btn->DS4ID[2];
		mapdata[MapName::Mapping_DS4_4] = btn->DS4ID[3];
		mapdata[MapName::Mapping_DS4_1_Disable] = btn->Disbale[0];
		mapdata[MapName::Mapping_DS4_2_Disable] = btn->Disbale[1];
		mapdata[MapName::Mapping_DS4_3_Disable] = btn->Disbale[2];
		mapdata[MapName::Mapping_DS4_4_Disable] = btn->Disbale[3];
